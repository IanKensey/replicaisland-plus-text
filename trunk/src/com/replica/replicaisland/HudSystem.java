...
public class HudSystem extends BaseObject {
    ...
    private DrawableText mTestTextDrawable;
    private Vector2 mTestTextLocation;
    ...
    public HudSystem() {
    	...
    	// TEMP FOR TESTING
        if (DebugText.TEXT_DEMO)
        {
	        mTestTextDrawable = new DrawableText(20);
	        mTestTextLocation = new Vector2();
	        mTestTextDrawable.setup(R.raw.generic_font, R.drawable.generic_font);
	        mTestTextDrawable.setString("Hello World!");
        }       
    	// TEMP FOR TESTING
    ...
    public void update(float timeDelta, BaseObject parent) 
    {
        ...
        if (DebugText.TEXT_DEMO)
        {
	        final float hX = params.gameWidth * 0.5f;
	        final float hY = params.gameHeight * 0.5f;
	        final float d = Math.min(hX, hY);
	    	mTestTextLocation.x = (float) (hX + (d * Math.sin(sSystemRegistry.timeSystem.getRealTime())));
	    	mTestTextLocation.y = (float) (hY + (d * Math.cos(sSystemRegistry.timeSystem.getRealTime())));
	    	render.scheduleForDraw(mTestTextDrawable, mTestTextLocation, SortConstants.HUD, false);
        }
    	// TEMP FOR TESTING
