...
public class Game extends AllocationGuard {
    ...
    public void bootstrap(Context context, int viewWidth, int viewHeight, int gameWidth, int gameHeight) {
        if (!mBootstrapComplete) {
            ...
            BaseObject.sSystemRegistry.fontLibrary = fontLibrary;
            FontInfo genericFont = fontLibrary.findOrLoadFont(R.raw.generic_font, R.drawable.generic_font);	// preload generic font
            if (DebugText.TEXT_DEMO)
            {
            	genericFont.dumpFont();
            }
